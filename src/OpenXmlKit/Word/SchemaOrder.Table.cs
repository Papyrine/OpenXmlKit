// <auto-generated>
// Generated by OpenXmlKit.Tests.SchemaOrderTests. Do not edit by hand.
//
// Read from the OpenXML SDK's own schema data, vendored under
// OpenXmlKit.Tests/SchemaData, which is where the SDK's code generator reads particle
// order from: every type with children whose composite type is a sequence, and whose base
// class is one the framework supplies rather than another element.
//
// The 28 containers that inherit children from another element are left out. Their
// bases are not all in the schema data - SdtElement is not there at all - so nothing could
// state where an inherited child goes. A container with no entry is one Place appends to,
...
{
    // Element namespace to prefix, so a child read back off a document can be matched
    // against the table below by the same key the schema writes.
