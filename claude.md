...
`SchemaOrder.Table.cs` is generated by `SchemaOrderTests.ShippedTableIsUpToDate`, which
regenerates it and fails when it is out of date — the same arrangement as the alias props, and for
the same reason. The input is the SDK's own schema data, the files its code generator reads
particle order from, vendored unmodified under `src/OpenXmlKit.Tests/SchemaData`. Unmodified on
purpose: trimming it to the four fields the generator reads would take it from 1.8 MB to 85 KB and
put a hand step between the SDK's data and this repo that nothing tests.

`SchemaOrderTests.TheTableAgreesWithTheSdk` is the semantic half. Assigning through a typed
property goes via `SetElement`, which places the child at its schema position, so for every
container the SDK emits typed properties for the SDK will state the order directly — 752 children
across 64 containers, assigned in reverse so a table echoing insertion order would not pass. It is
what caught the `SdtElement` problem below.
